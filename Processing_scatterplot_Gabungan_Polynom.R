...
p <- ggplot(all_data, aes(x = GRVI, y = NDVI, color = Type)) +
...
# Hitung model regresi polynomial
model_poly <- lm(NDVI ~ poly(GRVI, 2), data = all_data)
summary_model_poly <- summary(model_poly)
r_squared_poly <- summary_model_poly$r.squared

# Tambahkan regresi polynomial ke plot
p <- p + geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = FALSE, color = "black", size = 1) +
  annotate("text", x = max(all_data$GRVI, na.rm = TRUE), y = min(all_data$NDVI, na.rm = TRUE),
           label = paste("R² =", round(r_squared_poly, 2)), hjust = 1, vjust = -1, size = 6)

# Tampilkan plot
print(p)

# Tampilkan rumus regresi polynomial
cat("Rumus regresi polynomial: ")
cat(format(round(coef(model_poly)[1], 2)), " + ",
    format(round(coef(model_poly)[2], 2)), "*GRVI + ",
    format(round(coef(model_poly)[3], 2)), "*GRVI^2\n")
